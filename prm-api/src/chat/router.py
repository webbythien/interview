...
async def upload_files(files: List[UploadFile] = File(...)):
    ...
    for file in files:
        ...
        uploaded_files.append(
            UploadedFileInfo(url=file_url, type=file_type, filename=file.filename)
        )
...
async def send_mail():
    ...
    kwargs = {
        "to": {"email": "thiennhse162099@fpt.edu.vn", "name": "Thien"},
        "subject": subject,
...
async def get_joined_groups(
    ...
    try:
        ...
        latest_message_subquery = (
            ...
            .group_by(Conversation.reciever_id)
            .subquery()
        )
        ...
        member_count_subquery = (
            db.query(
                ...
                func.count(func.distinct(GroupConversation.user_uuid)).label(
                    "member_count"
                ),
            )
            .group_by(GroupConversation.group_id)
            .subquery()
        )
        ...
        joined_groups_query = (
            ...
            .outerjoin(
                latest_message_subquery,
                Group.id == latest_message_subquery.c.reciever_id,
            )
            .outerjoin(
                member_count_subquery, Group.id == member_count_subquery.c.group_id
            )
            .group_by(
                Group.id,
                member_count_subquery.c.member_count,
            ...
            .order_by(
                desc(latest_message_subquery.c.latest_message_time),
        ...
        for group, member_count, latest_message_time in joined_groups:
            ...
            recent_senders_query = (
                db.query(GroupConversation.username)
                .filter(GroupConversation.group_id == group_id)
                .order_by(desc(GroupConversation.created_at))
                .limit(3)
            )
            ...
            if latest_message_time:
                ...
                recent_message = (
                    most_recent_message.message if most_recent_message else None
                )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to fetch joined groups",
        )
...
async def get_not_joined_groups(
    ...
    try:
        ...
        joined_group_ids = (
            db.query(GroupConversation.group_id)
            .filter(GroupConversation.user_uuid == user_uuid)
            .subquery()
        )

        # Query to get groups that the user has not joined along with member counts
        not_joined_groups_query = (
            db.query(
                Group, func.count(GroupConversation.user_uuid).label("member_count")
            )
        ...
        )  # Sort by group ID in descending order
        ...
        for group, member_count in not_joined_groups:
            ...
            recent_message_query = (
                db.query(func.max(Conversation.created_at).label("last_message_time"))
                .filter(Conversation.reciever_id == group_id)
                .scalar()
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to fetch not joined groups",
        )
...
async def join_group(request: JoinGroupRequest, db: Session = Depends(get_db)):
    try:
        ...
        new_member = GroupConversation(
            group_id=request.group_id, user_uuid=request.uuid, username=request.username
        )
        db.add(new_member)
        db.flush()

        group_details: Group = (
            db.query(Group).filter(Group.id == request.group_id).first()
        )
        ...
        member_count = (
            db.query(GroupConversation)
            .filter(GroupConversation.group_id == request.group_id)
            .count()
        )
...
async def check_user_in_group(
    group_id: int, user_uuid: str, db: Session = Depends(get_db)
):
    try:
...
async def get_group_messages(
    ...
    start_id: Optional[int] = Query(
        None, description="ID to start fetching messages from"
    ),
    db: Session = Depends(get_db),
):
    try:
        # Base query to get messages for the specified group_id
        messages_query = db.query(Conversation).filter(
            Conversation.reciever_id == group_id
        )
        ...
        messages = (
            messages_query.order_by(Conversation.created_at.desc())
            .offset(offset)
            .limit(limit + 1)
            .all()
        )
...
def format_message(message, db):
    ...
    sender = (
        db.query(GroupConversation)
        .filter(GroupConversation.user_uuid == message.sender_uuid)
        .first()
    )
    ...
    message_data = {
        ...
        "sender_uuid": (
            sender.user_uuid if sender else None
        ),  # Add sender UUID if available
    }
    ...
    if message.subtype == "img":
        images = (
            db.query(ConversationImage)
            .filter(ConversationImage.conversation_id == message.id)
            .all()
        )
        ...
        if docs:
            message_data["doc"] = {"name": docs[0].name, "link": docs[0].link}

import { Box, Divider, IconButton, Link, Stack, Typography, Menu, MenuItem, Avatar } from '@mui/material';
            ...
            <Box p={1.5} sx={{
                ...
                <Stack spacing={2}>
                    <Stack p={2} spacing={3} direction='row' alignItems='center'
                        ...
                        <IconButton>
                            <a href={el.doc.link} download={el.doc.name}>
                                <DownloadSimple />
                            </a>
                        </IconButton>
                    </Stack>
...
const LinkMsg = ({ el, menu }) => {
    ...
    useEffect(() => {
        const fetchMetaData = async () => {
            try {
                const response = await fetch(`/api/metadata?url=${encodeURIComponent(el.link)}`);
                ...
                <Stack spacing={2}>
                    <Stack p={2} spacing={3} alignItems='start'
                        ...
                        {loading ? (
                            ...
                            <>
                                {metaData.image && <img src={metaData.image} alt={metaData.title} style={{ maxHeight: 210, borderRadius: '10px' }} />}
                                <Stack spacing={2}>
                                    <Typography variant='subtitle2'>{metaData.title}</Typography>
                                    <Typography variant='subtitle2' sx={{ color: theme.palette.primary.main }}
                                        component={Link} to={el.link}>{el.link}</Typography>
                                </Stack>
